...
description: >-
  ...
  host port / build dirs with `fix-nuxeo-web-ui-bug/scripts/new-ticket-workspace.sh`,
  hands each child agent its `$NX_*` overrides, batches the run into waves that
  fit the machine's Docker memory budget, then reports per-ticket outcome and PR
  links. Use when asked to "fix all children of <epic>", "run multiple bug fixes
  at once", "one agent per ticket", "work on these tickets in parallel", or to
  plan capacity for a batch of WEBUI-/ELEMENTS- tickets.
---
...
`/tmp/dist-*`, `~/Desktop/jira-evidence/<TICKET-ID>/fix` — becomes a per-run variable here. The
override list is in [The `$NX_*` contract](#the-nx_-contract) and you must pass it to every child
agent.
...
These numbers are **measured on a 16-CPU / 64 GB machine**, not estimates. Re-check the Docker
figure on yours (`docker info --format '{{.MemTotal}}'`) and treat the result as a hard planning
constraint:
...
The workspace script ships with the single-ticket skill, because a solo run needs it too. Call it
once per ticket and base:
...
WebUI/
  ...
  worktrees/WEBUI-2170/lts-2025/        # web-ui/ + elements/ + env.sh
  worktrees/WEBUI-2170/maintenance-3.1.x/
```
...
The script rejects a root inside the reference clone for that reason.
...
script **stops and asks you to inspect it** rather than guessing. Check that clone for work worth
keeping, then re-run with `--force` to rebuild it from scratch.
...
Then, in the child agent: **set its root to the workspace** and source the environment before
anything else.
...
| `$NX_EVIDENCE` | `~/Desktop/jira-evidence/<TICKET-ID>/fix` | screenshots, videos, repro dir |
...
one, and the bases of a ticket never collide:

```
PORT_START = 8100 + (<ticket-number> % 250) * 3 + <base-offset>
# lts-2025 0, maintenance-3.1.x 1, maintenance-3.0.x 2
```

Three consecutive ports per ticket, one per base, keeps every starting port inside `8100..8849`.
...
second would otherwise both see the port as unclaimed and both take it. A run that cannot take the
lock within 30s **fails** rather than risk sharing a port — if you hit that, another run is mid-flight,
so retry in a moment.

## Skills overlay (temporary — while the bases are behind)

A base branch can track an **older** copy of these skills, so a fresh clone of it would hand stale
instructions to the agent working in it. The script therefore copies the current `.cursor/skills/`
into each workspace and hides it from git:
...
Also **symlink the skills into your personal folder** (see the README's
[one-time setup](../README.md)) so an agent reads the current instructions *before* any workspace
exists:
...
3. its workspace path(s), and the instruction to set its root there and `. env.sh` first;
...
6. the instruction to follow [`fix-nuxeo-web-ui-bug`](../fix-nuxeo-web-ui-bug/SKILL.md) end-to-end,
   through its Phase 10 clean-up and report.
...
- **Empty `fixVersions` doubles the batch.** Children of an epic often have **no** `fixVersions`, and
  the core skill's rule is "every fix ships on **both** bases". An agent handed an empty
...
- **`ELEMENTS-*` children live in a different repo.** e.g. `ELEMENTS-1611` lands in
  ...
  **`3.0.x` is deprecated**, so ignore that fixVersion rather than opening a branch for it. The
  ...
  `nuxeo-web-ui`.
- **Not every accessibility child is a client-side bug with a minimal fix.** "PDF Documents: No
  accessibility tags on export" and "Integrated Video Player: no captions" need backend or
  third-party work, not a Web UI patch. Route those to
  [`jira/raise-backend-jira-ticket`](../jira/raise-backend-jira-ticket/SKILL.md) rather than burning a
  workspace forcing a client fix. Decide this in Phase A, on the ticket text.
