...
shift
# Anchored on purpose. TICKET is interpolated into TDIR, which --remove passes to `rm -rf`,
# and into the Docker container name, so anything that is not strictly PROJECT-1234 has to be
# rejected here: a glob such as [A-Z]*-[0-9]* would happily accept `WEBUI-1/../../elsewhere`.
...
# copy_node_modules <source node_modules> <destination node_modules>
copy_node_modules() {
  local src=$1 dest=$2
  [ -d "$src" ] || return 1
  [ -d "$dest" ] && return 0
  ...
  if cp -Rc "$src" "$dest" 2>/dev/null; then
    note "APFS clone of $src"
  else
    cp -R "$src" "$dest"
    note "plain copy of $src (filesystem has no clonefile support)"
  fi
}

step "Populating node_modules"
if ! copy_node_modules "$MAIN_REPO/node_modules" "$WT/node_modules"; then
  note "no reference node_modules — running npm ci (this takes a few minutes)"
  (cd "$WT" && npm ci)
fi

# The elements clone needs its own dependencies too. Web UI imports @nuxeo/nuxeo-ui-elements
# straight from that checkout, and those modules resolve their own imports (lit, iron-*, ...)
# through the *elements* node_modules — nuxeo-elements is a monorepo, so ui/ has one of its own
# on top of the root one. Without them the dev server cannot resolve those bare specifiers, the
# test page dies mid-load and Web Test Runner reports only "the browser disconnected".
if [ "$ELEMENTS_TARGET" = "$EL" ]; then
  step "Populating elements node_modules"
  copied=0
  for sub in . ui storybook; do
    src=$MAIN_ELEMENTS/$sub/node_modules
    [ -d "$src" ] || continue
    copy_node_modules "$src" "$EL/$sub/node_modules" && copied=1
  done
  if [ "$copied" = 0 ]; then
    note "no reference elements node_modules — running npm ci (this takes a few minutes)"
    (cd "$EL" && npm ci)
  fi
fi
...
# Keyed on ELEMENTS_TARGET alone: gating this on the reference clone having dev symlinks
# skipped the repoint entirely whenever node_modules came from `npm ci` instead of a copy,
# leaving the workspace silently reading registry packages rather than the elements clone.
if [ -n "$ELEMENTS_TARGET" ]; then
...
# Carrying on without the lock would give up the one guarantee this section exists to provide:
# two runs starting together would both see the same port as free and both claim it. A workspace
# that quietly shares a port with another ticket is worse than no workspace, so stop instead.
[ "$locked" = 1 ] ||
  die "could not take the port lock at $LOCK after 30s. Another run is probably mid-flight — retry
     in a moment, or remove that directory if you are sure no other run is active."
...
# Emit single-quoted values: NX_TICKETS_ROOT / NX_EVIDENCE_ROOT are user-supplied paths, and an
# unquoted one containing a space or a shell metacharacter produces an env.sh that cannot be
# sourced (or, worse, one that executes part of the path).
shq() { printf "'%s'" "$(printf '%s' "$1" | sed "s/'/'\\\\''/g")"; }

cat > "$TDIR/env.sh" <<EOF
# Generated by new-ticket-workspace.sh — source this before working on $TICKET.
export NX_TICKET=$(shq "$TICKET")
export NX_BASE=$(shq "$BASE")
export NX_WT=$(shq "$WT")
export NX_ELEMENTS=$(shq "$ELEMENTS_TARGET")
export NX_EVIDENCE=$(shq "$EVIDENCE")
export NX_CONTAINER=$(shq "$CONTAINER")
export NX_PORT=$(shq "$PORT")
export NX_URL=$(shq "http://localhost:$PORT/nuxeo")
export NX_DIST_PATCHED=$(shq "$DIST_PATCHED")
export NX_DIST_UNPATCHED=$(shq "$DIST_UNPATCHED")
EOF
