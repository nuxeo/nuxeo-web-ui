...
class S3Provider {
  ...
  _handUploadError(error, callback) {
    if (this.files.every((file) => file.error || file.complete)) {
      callback({ type: 'batchFinished', batchId: this.batchId });
    } else if (this.files.every((file) => file.error)) {
  ...
  upload(files, callback) {
    this._ensureBatch().then(() => {
      ...
      for (let i = 0; i < files.length; ++i) {
        ...
        promises.push(this._upload(file, callback).catch((error) => this._handUploadError(error, callback)));
      }
