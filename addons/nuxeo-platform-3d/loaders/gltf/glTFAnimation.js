...
glTFInterpolator.prototype.interp = function(t) {
  ...
  if (t == this.keys[0]) {
    ...
    for (i = 0; i < this.count - 1; i++) {
      ...
      if (t >= key1 && t <= key2) {
        if (this.isRot) {
          this.quat1.set(this.values[i * 4], this.values[i * 4 + 1], this.values[i * 4 + 2], this.values[i * 4 + 3]);
          this.quat2.set(
            this.values[(i + 1) * 4],
            this.values[(i + 1) * 4 + 1],
            this.values[(i + 1) * 4 + 2],
