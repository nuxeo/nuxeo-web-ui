...
import { GLTFLoaderUtils } from './glTFLoaderUtils.js';
...
glTFLoader.prototype.load = function(url, callback) {
  ...
  VertexAttributeDelegate.prototype.bufferResourceAvailable = function(glResource, ctx) {
    ...
    if (semantic == 'POSITION') {
      ...
      geom.geometry.setAttribute('position', new THREE.BufferAttribute(floatArray, 3));
      ...
      geom.geometry.setAttribute('normal', new THREE.BufferAttribute(floatArray, 3));
      ...
      geom.geometry.setAttribute('uv', new THREE.BufferAttribute(floatArray, nComponents));
    } else if (semantic == 'WEIGHT') {
      nComponents = componentsPerElementForGLType(attribute.type);
      floatArray = new Float32Array(glResource, 0, attribute.count * nComponents);
      geom.geometry.setAttribute('skinWeight', new THREE.BufferAttribute(floatArray, nComponents));
    } else if (semantic == 'JOINT') {
      nComponents = componentsPerElementForGLType(attribute.type);
      floatArray = new Float32Array(glResource, 0, attribute.count * nComponents);
      geom.geometry.setAttribute('skinIndex', new THREE.BufferAttribute(floatArray, nComponents));
    }
  };
    ...
    this.primitives.forEach(function(primitive) {
      ...
      if (!that.skin) {
        ...
        if (material instanceof THREE.ShaderMaterial) {
          var shader = new glTFShader(material, materialParams, threeMesh, theLoader.rootObj);
          glTFShaders.add(shader);
        }
      }
    });
  };
    ...
    threeJSMaterialType: {
      value: function(materialId, material, params) {
        ...
        if (khr_material) {
          ...
          var technique = material.technique ? this.resources.getEntry(material.technique) : null;
        ...
        // hack hack hack
        if (params.map && params.map.sourceFile.toLowerCase().indexOf('.png') != -1) params.transparent = true;
          ...
          if (primitiveDescription.mode === WebGLRenderingContext.TRIANGLES) {
            ...
            theLoader.scheduleLoad(function(data) {
              ...
              if (alreadyProcessedIndices) {
                data.indicesDelegate.resourceAvailable(alreadyProcessedIndices, data.indicesContext);
              }
            }, loaddata);
            ...
            allAttributes.forEach(function(semantic) {
              ...
              theLoader.scheduleLoad(function(data) {
                ...
                if (alreadyProcessedAttribute) {
                  data.vertexAttributeDelegate.resourceAvailable(alreadyProcessedAttribute, data.attribContext);
                }
              }, loaddata);
            }, this);
          }
    ...
    handleNode: {
      value: function(entryID, description, userInfo) {
        ...
        if (m) {
          ...
          var scale = s ? new THREE.Vector3(s[0], s[1], s[2]) : new THREE.Vector3(1, 1, 1);
        ...
        if (
          ...
          description.extensions.KHR_materials_common &&
              ...
              for (meshID in glTF.meshInstances) {
                ...
                mesh.primitives.forEach(function(primitive) {
                  ...
                  if (threeMesh && dobones) {
                    ...
                    threeMesh.bind(new THREE.Skeleton(bones, boneInverses, false), skin.bindShapeMatrix);
                  ...
                  if (threeMesh) {
                    ...
                    if (material instanceof THREE.ShaderMaterial) {
                      ...
                      var glTFShader = new glTFShader(material, materialParams, threeMesh, theLoader.rootObj);
                      glTFShaders.add(glTFShader);
                    }
                  }
                }, this);
              }
      ...
      value: function(entryID, description, userInfo) {
        ...
        params.forEach(function(param) {
          ...
          theLoader.scheduleLoad(function(data) {
            ...
            if (alreadyProcessedAttribute) {
              data.animationParameterDelegate.resourceAvailable(alreadyProcessedAttribute, data.paramContext);
            }
          }, loaddata);
        }, this);
    ...
    handleSkin: {
      value: function(entryID, description, userInfo) {
        ...
        theLoader.scheduleLoad(function(data) {
          ...
          if (alreadyProcessedAttribute) {
            data.inverseBindMatricesDelegate.resourceAvailable(alreadyProcessedAttribute, data.context);
          }
        }, loaddata);
...
glTFLoader.prototype.checkComplete = function() {
  if (
    ...
    glTFShaders.bindShaderParameters(this.rootObj);
