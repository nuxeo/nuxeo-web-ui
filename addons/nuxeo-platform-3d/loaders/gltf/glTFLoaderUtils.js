...
let GLTFLoaderUtils = Object.create(Object, {
  ...
  _handleWrappedBufferViewResourceLoading: {
    value: function(wrappedBufferView, delegate, ctx) {
      ...
      var range = [
        byteOffset,
        this._elementSizeForGLType(wrappedBufferView.componentType, wrappedBufferView.type) * wrappedBufferView.count +
          byteOffset,
      ];
  ...
  getFile: {
    value: function(request, delegate, ctx) {
      ...
      this._handleRequest(
        { id: request.id, uri: request.uri, range: [0], type: 'text', delegate: delegate, ctx: ctx },
        null,
      );
