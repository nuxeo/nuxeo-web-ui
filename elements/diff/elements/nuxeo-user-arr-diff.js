          ...
          <template is="dom-repeat" items="[[_getArrayDelta(delta, originalValue, newValue, hideAdditions, hideDeletions)]]" as="arrdelta">
            <template is="dom-if" if="[[_showArrayItem(arrdelta, showAll)]]">
              <div class\$="item [[arrdelta.change]]">
                <nuxeo-user-diff delta="[[arrdelta.value]]" original-value="[[arrdelta.originalValue]]" new-value="[[arrdelta.newValue]]" unified="[[unified]]" schema="[[_getPropertySchema(schema, property)]]" type="directoryEntry" show-all="[[showAll]]" level="[[level]]" hide-deletions="[[hideDeletions]]" hide-additions="[[hideAdditions]]" label="[[arrdelta.index]]:" is-array-item="" display-label=""></nuxeo-user-diff>
              </div>
            </template>
          </template>
        </div>
        ...
        <template is="dom-if" if="[[_isArray(delta)]]">
          <div class="array simple">
            <template is="dom-repeat" items="[[originalValue]]" as="value">
              <div class="item">
                <nuxeo-user-diff original-value="[[value]]" schema="[[_getPropertySchema(schema, property)]]" type="directoryEntry" unified="[[unified]]" show-all="[[showAll]]" level="[[level]]" hide-deletions="[[hideDeletions]]" hide-additions="[[hideAdditions]]" label="[[index]]:" is-array-item="" display-label=""></nuxeo-user-diff>
              </div>
            </template>
          </div>
        </template>
