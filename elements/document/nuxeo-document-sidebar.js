...
Polymer({
  ...
  attached() {
    this.resizeObserver =
      this.resizeObserver ||
      new ResizeObserver(() => {
        if (window.matchMedia('(min-width: 1024px)').matches && this.expandedWidth) {
          this.shadowRoot.querySelector('.scroller').style.setProperty('min-width', this.expandedWidth);
        } else {
          this.shadowRoot.querySelector('.scroller').style.setProperty('min-width', 'initial');
        }
      });
