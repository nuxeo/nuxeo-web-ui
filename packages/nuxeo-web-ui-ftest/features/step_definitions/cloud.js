...
Then('I can edit {string} provider to:', async function(currentName, newDetails) {
  const administrationEle = await this.ui.administration;
  await administrationEle.cloudServices.waitForVisible();
  const cloudServicesEle = await administrationEle.cloudServices;
...
Then('I can see the nuxeo-cloud-consumers page', async function() {
  const cloudServicesEle = await this.ui.administration.cloudServices;
  await cloudServicesEle.waitForVisible();
  const NuxeoCloudConsumersPage = await cloudServicesEle.nuxeoCloudConsumers.waitForVisible();
  if (!NuxeoCloudConsumersPage) {
