...
export default class Collections extends BasePage {
  waitForHasCollection(name, reverse) {
    ...
    driver.waitUntil(
      () => {
        const collections = el.$$('span.collection-name');
        if (reverse) {
  ...
  select(name) {
    const el = this.el.$('#collectionsList span.title');
  ...
  waitForHasMember(doc, reverse) {
    ...
    driver.waitUntil(
      () => {
        const members = el.$$('#membersList .list-item-title');
        if (reverse) {
  ...
  removeMember(doc) {
    const members = this.el.$$('#membersList');
