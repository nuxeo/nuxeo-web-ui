...
export default class Recents extends BasePage {
  get nbItems() {
    const items = this.el.$$('#recentDocumentsList .list-item');
    let count = 0;
    items.forEach((item) => {
      if (item.isVisible()) {
        count++;
  ...
  select(name) {
    const entries = this.el.$$('#recentDocumentsList .list-item-title');
  ...
  waitForHasMember(title, reverse) {
    driver.waitUntil(
      () => {
        const members = this.el.$$('#recentDocumentsList .list-item-title');
        if (reverse) {
