...
fixtures.collections = {
  addToNewCollection: (document, colName) => nuxeo.operation('Collection.Create').params({
    name: colName,
  }).execute().then(col => nuxeo.operation('Document.AddToCollection').input(document).params({
    collection: col,
  }).execute()),
