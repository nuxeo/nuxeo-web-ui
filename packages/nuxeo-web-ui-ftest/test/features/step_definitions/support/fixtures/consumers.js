...
fixtures.clients = {
  create: (client) => {
    ...
    return nuxeo.request(`${endPoint}${clientId}`).get().then((_client) => {
      ...
      if (response.response && response.response.status && response.response.status === 404) {
        ...
        throw `unable to get client "${clientId}"`;
      }
    });
  },
  delete: client => nuxeo.request(`${endPoint}${client}`).delete().then(() => delete global.clients[client]),
};

After(() => Promise.all(Object.keys(global.clients)
  .map(client => fixtures.clients.delete(client))));
