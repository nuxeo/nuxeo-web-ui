...
suiteSetup(async () => {
  tmpl = await loadDomModuleTemplate('/elements/nuxeo-home.html', 'nuxeo-home');
});

suite('nuxeo-home', () => {
  suite('WCAG H2: thumbnail combined with title in one link across all dashboard tables', () => {
    test('has a thumbnail in each dashboard section', () => {
      const thumbnails = queryAllDeep(tmpl.content, 'nuxeo-document-thumbnail');
      expect(thumbnails.length).to.be.at.least(
        3,
